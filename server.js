...
// ==========================================
// 🔴 配置你的云端账本 (请把你的 ID 和 Key 填在引号里)
// ==========================================
const JSONBIN_ID = "69a1c1b1d0ea881f40df6830"; 
const JSONBIN_KEY = "$2a$10$HQskEt9Uozt073YApnnoPeu2eVFF/tdKStQPQSLTWpqajB0TFORgu"; 
...
// 完整的18个风格库
const themes = {
    tech: { color:"#0064d2", bg:"#f0f6fc", font:"Roboto, sans-serif" },
    fashion: { color:"#b76e79", bg:"#ffffff", font:"'Playfair Display', serif" },
    auto: { color:"#d63031", bg:"#fff5f5", font:"'Segoe UI', Verdana, sans-serif" },
    home: { color:"#27ae60", bg:"#f1f8e9", font:"'Open Sans', sans-serif" },
    industrial: { color:"#333", bg:"#eee", font:"Arial Black, sans-serif" },
    weapon: { color:"#4b5320", bg:"#f0f2eb", font:"Arial, Helvetica, sans-serif" },
    auto_parts: { color: "#d32f2f", bg: "#121212", font: "'Segoe UI', Roboto, Helvetica, Arial, sans-serif" },
    retro: { color:"#5d4037", bg:"#f5f1e8", font:"'Courier New', monospace" },
    adult_pro: { color:"#9c27b0", bg:"#f3e5f5", font:"'Montserrat', sans-serif" },
    xmas_pro: { color: "#2e7d32", bg: "#f1f8e9", font: "'Segoe UI', sans-serif" },
    steam: { color: "#2c3e50", bg: "#f9f9f9", font: "Arial, sans-serif" },
    men_power: { color: "#b71c1c", bg: "#f5f5f5", font: "Arial, Helvetica, sans-serif" },
    cute_pet: { color: "#ff99cc", bg: "#f8f8f8", font: "'Comic Sans MS', cursive, sans-serif" },
    halloween: { color:"#ff6600", bg:"#1a0524", font:"'Verdana', sans-serif" },
    scifi: { color: "#00f2ff", bg: "#05080f", font: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" },
    blackfriday: { color:"#000", bg:"#ffff00", font:"Arial Black, sans-serif" },
    valentine: { color:"#e91e63", bg:"#fff0f6", font:"'Comic Sans MS', cursive" },
    july4: { color:"#34495e", bg:"#fff", font:"Impact, sans-serif" }
};
...
app.post('/api/activate', async (req, res) => {
    ...
    if (!deviceId || !licenseKey) return res.status(400).json({ error: "参数缺失" });

    try {
        const binUrl = `https://api.jsonbin.io/v3/b/${JSONBIN_ID}`;
        const headers = { 'X-Master-Key': JSONBIN_KEY };
        
        let response = await axios.get(binUrl + '/latest', { headers });
        let data = response.data.record; 

        if (!data.codes || !data.codes[licenseKey]) {
            return res.status(400).json({ error: "无效的激活码，或已被使用！" });
        }

        const daysToAdd = data.codes[licenseKey];
        let currentExpiry = data.users[deviceId] ? data.users[deviceId] : Date.now();
        ...
        delete data.codes[licenseKey]; 
        data.users[deviceId] = newExpiry; 

        await axios.put(binUrl, data, { headers: { ...headers, 'Content-Type': 'application/json' } });

        res.json({ success: true, expiry: newExpiry, message: `充值成功！增加了 ${daysToAdd} 天。` });

    } catch (error) {
        console.error(error);
        res.status(500).json({ error: "云端服务器查账失败，请检查密钥配置" });
    }
});
...
app.post('/api/generate', async (req, res) => {
    ...
    if (!clientKey) return res.status(401).json({ error: "未检测到设备ID" });

    try {
        // 1. 去 JSONBin 查账
        const binUrl = `https://api.jsonbin.io/v3/b/${JSONBIN_ID}/latest`;
        const checkRes = await axios.get(binUrl, { headers: { 'X-Master-Key': JSONBIN_KEY } });
        const userExpiry = checkRes.data.record.users[clientKey];

        // 2. 检查是否过期 (如果你自己想白嫖，可以把你的设备ID加在这里跳过验证)
        if (!userExpiry || userExpiry < Date.now()) {
            return res.status(403).json({ error: "您的会员已过期或未激活，请充值续费！" });
        }
        ...
        if(curTheme === 'auto_parts') {
            ...
            extraCSS = `.con { border: 1px solid #333; border-top: 5px solid #d32f2f; background: #1a1a1a; box-shadow: 0 10px 30px #000; } .tit-box { background: transparent; padding: 30px 20px; border-bottom: none; box-shadow: none; position: relative; } .tit { font-size: 1.8rem; font-weight: 800; color: #fff; text-transform: uppercase; letter-spacing: 1px; text-align:center;} .tit-box::after { content: ''; display: block; width: 60px; height: 4px; background: #d32f2f; margin: 15px auto 0; } .stg, .th { border: 1px solid #444; background: #121212; } .th:hover { border-color: #d32f2f; } .desc { border-top: none; padding: 20px; } .tabs { margin-top:30px; border-top: 5px solid #d32f2f; background: #000; } .tab { border-bottom: 1px solid #222; } .thd { background: #111 !important; color: #fff !important; font-weight: bold; text-transform: uppercase; padding: 16px 20px; font-size:1.15rem; transition: 0.3s; border-left:5px solid #000; } .thd:hover { background: #222 !important; color: #d32f2f !important; } .tb { background: #1a1a1a; color: #ccc; border-top: none; } .tc:checked~.thd { background: #d32f2f !important; color: #fff !important; border-left: 5px solid #8b0000; } .hazard-bar { height: 10px; width: 100%; background: repeating-linear-gradient(45deg, #111, #111 10px, #b71c1c 10px, #b71c1c 20px); margin-bottom: 0; }`;
            ...
            extraCSS = `@keyframes snow { 0% {background-position: 0 0, 0 0;} 100% {background-position: 500px 1000px, 400px 400px;} } .snow-overlay { position: absolute; top:0; left:0; width:100%; height:100%; pointer-events:none; z-index:99; background-image: radial-gradient(white 2px, transparent 2px), radial-gradient(white 1px, transparent 1px); background-size: 50px 50px; animation: snow 10s linear infinite; opacity: 0.6; }`;
            preContent = '<div class="snow-overlay"></div>';
        } else if(curTheme === 'steam') {
             bgStyle = `background-color: #f9f9f9; color: #2c3e50;`;
             extraCSS = `.pipe-decoration { height: 5px; background: linear-gradient(to right, #bdc3c7, #95a5a6, #bdc3c7); margin: 20px 0; position: relative; } .pipe-decoration::before, .pipe-decoration::after { content: ""; position: absolute; width: 15px; height: 15px; background-color: #95a5a6; border-radius: 50%; top: -5px; } .pipe-decoration::before { left: 0; } .pipe-decoration::after { right: 0; } .con { border: 2px solid #7f8c8d; border-radius: 5px; box-shadow: 0 5px 15px rgba(0,0,0,0.1); background: rgba(236, 240, 241, 0.9); } .tit-box::before, .tit-box::after { content: "⚙️"; position: absolute; top: 50%; transform: translateY(-50%); font-size:24px; } .tit-box::before { left: 20px; } .tit-box::after { right: 20px; } .th { border: 2px solid #95a5a6; background: #000; background-size: contain; background-repeat: no-repeat; background-position: center; }`;
             ...
             extraCSS = `@keyframes bounce { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-10px); } } .paw-decoration { display: flex; justify-content: center; gap: 15px; margin: 20px 0; } .paw { font-size: 24px; color: #ff99cc; animation: bounce 1.5s infinite; } .paw:nth-child(2) { animation-delay: 0.2s; color: #66ccff; } .paw:nth-child(3) { animation-delay: 0.4s; color: #ffcc66; } .con { border-radius: 15px; border: 2px solid #e6e6e6; } .tit-box { background: #fff; border-bottom: 3px dashed #66ccff; border-radius: 15px 15px 0 0; } .th { border-radius: 8px; }`;
            ...
            extraCSS = `.con { border: 2px solid #ff6600; box-shadow: 0 0 20px rgba(255, 102, 0, 0.2); background: rgba(20, 10, 30, 0.95); border-radius: 10px; } .tit-box { background: linear-gradient(135deg, #ff6600 0%, #cc0000 100%); border-bottom: 4px solid #6600cc; clip-path: polygon(0 0, 100% 0, 100% 85%, 50% 100%, 0 85%); padding-bottom: 30px; position: relative; } .tit-box::before { content: '🦇'; position: absolute; left: 15px; top: 15px; font-size: 24px; animation: float 3s ease-in-out infinite; } .tit-box::after { content: '🦇'; position: absolute; right: 15px; top: 15px; font-size: 24px; animation: float 3s ease-in-out infinite; animation-delay: 1.5s; } @keyframes float { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-10px); } } .stg, .th { border: 1px solid #ff6600; background-color: #000; background-size: contain; background-repeat: no-repeat; background-position: center; } .th:hover { border-color: #9900ff; box-shadow: 0 0 8px #9900ff; } .desc { border-left: 3px solid #ff6600; background: rgba(255, 102, 0, 0.05); } .thd { color: #ff6600 !important; font-family: 'Verdana', sans-serif; letter-spacing: 1px; } .thd:hover { background: #2a1035 !important; }`;
            ...
            extraCSS = `.con { border: 1px solid #00f2ff; box-shadow: 0 0 15px rgba(0,242,255,0.15); background: rgba(11,12,21,0.9); } .tit-box { background: linear-gradient(to right, rgba(0,242,255,0.1), rgba(189,0,255,0.1)); border-bottom: 2px solid #00f2ff; color: #fff; position: relative; overflow: hidden; clip-path: polygon(0 0, 100% 0, 100% 85%, 98% 100%, 2% 100%, 0 85%); } .tit { letter-spacing: 2px; text-transform: uppercase; text-shadow: 0 0 10px #00f2ff; } .tit-box::before { content: ''; position: absolute; top: 0; left: -100%; width: 50%; height: 100%; background: linear-gradient(90deg, transparent, rgba(0,242,255,0.4), transparent); animation: scan 4s linear infinite; } @keyframes scan { 0% {left:-100%} 100% {left:200%} } .stg, .th { border: 1px solid #00f2ff; background-color: #000; box-shadow: 0 0 5px rgba(0,242,255,0.3); background-size: contain; background-repeat: no-repeat; background-position: center; } .th:hover { box-shadow: 0 0 10px #bd00ff; border-color: #bd00ff; } .desc { border-left: 3px solid #bd00ff; background: rgba(0,242,255,0.02); } .warning-box { border: 1px solid #00ff9d; background: rgba(0,255,157,0.1); color: #00ff9d; padding: 10px; }`;
        }
...
<div class="tabs">
    ...
    <div class="tab"><input type="checkbox" id="t5" class="tc"><label for="t5" class="thd">${pol.hFeed}</label><div class="tb">${pol.bFeed}</div></div>
        ...
        let relatedHTML = "";
        if (includeRelated) {
            const relTitle = pol.hRel || "You may also like";
            relatedHTML = `<div style="margin-top:30px; border-top:2px solid #eee; padding-top:20px;"><div style="font-weight:bold; font-size:1.2rem; color:${cfg.color}; margin-bottom:15px; text-align:center;">${relTitle}</div><div class="rel-content" style="min-height: 150px; background:#f9f9f9; text-align:center; color:#999; padding:20px;"></div></div>`;
        }
...
<div class="ebay-app">
    <div class="con">
        ...
        console.error(e);
        res.status(500).json({ error: "服务器验证失败，请重试" });
    }
});
